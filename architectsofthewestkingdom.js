...
function (dojo, declare) {
    return declare("bgagame.architectsofthewestkingdom", ebg.core.gamegui, {
        ...
        adaptViewportSize : function() {
            ...
            var rowWidth = 2000;
            var zoom = this.zoomScales[this.zoomIndex];
        ...
        */
        ...
        setup: function( gamedatas )
        {
            ...
            for( var player_id in gamedatas.players )
            {
            	...
            	dojo.query('.res_'+player['id']+'_'+7).forEach(function(selectTag){ selectTag.innerHTML += "/6" ; }); 
            	...
            	dojo.place(this.format_block('jstpl_marker', {id: "cathedral_"+player['id'], color: player['color'] }), $('cathedral'+player['cathedral']));
            	dojo.place(this.format_block('jstpl_marker', {id: "virtue_"+player['id'], color: player['color'] }), $('virtue'+player['virtue']));
    			...
    			dojo.style("res8_"+player['id'], {"background-position": (25*type)+"% 0%"});
    			var name = _(this.helper.bosses[player['type']]['name']);
    		    dojo.query('#name'+player['id']).forEach(function(selectTag){ selectTag.innerHTML = name; });     			
            ...
            {
            	dojo.place(this.format_block('jstpl_bmcard', {id:gamedatas.blackmarket1.id, posx:(gamedatas.blackmarket1.type-1) *11.11, posy:50  }), $("phblackmarket1")); 
            }
            if(gamedatas.blackmarket2 != null)
            {
            	 dojo.place(this.format_block('jstpl_bmcard', {id:gamedatas.blackmarket2.id, posx:(gamedatas.blackmarket2.type-1) *11.11, posy:0  }), $("phblackmarket2")); 
            }
            
            if(gamedatas.rewardnb>0)
            {
            	dojo.query('#rewardcpt').forEach(function(selectTag){ selectTag.innerHTML = gamedatas.rewardnb; }); 
            }
            else
            {
        	...
        	dojo.query('#taxcpt').forEach(function(selectTag){ selectTag.innerHTML = gamedatas.tax; }); 
            ...
            {
            	dojo.query('#txt'+trad_id).forEach(function(selectTag){ selectTag.innerHTML = _(boards[trad_id]); }); 
            }
            for( var trad_id in this.helper.tooltips )
            {
            	this.addTooltipHtml( trad_id, _(this.helper.tooltips[trad_id]),1000);
            }
        	dojo.query('.or').forEach(function(selectTag){ selectTag.innerHTML = _(translates["or"]); }); 
        	dojo.query('.per').forEach(function(selectTag){ selectTag.innerHTML = _(translates["per"]); }); 
        	dojo.query('.actionper').forEach(function(selectTag){ selectTag.innerHTML = _(translates["actionper"]); }); 
        	dojo.query('.most').forEach(function(selectTag){ selectTag.innerHTML = _(translates["most"]); }); 
        	dojo.query('.yourhand').forEach(function(selectTag){ selectTag.innerHTML = _(translates["yourhand"]); }); 
        	dojo.query('.selectCard').forEach(function(selectTag){ selectTag.innerHTML = _(translates["selectCard"]); }); 
        	...
        	if(gamedatas.score.length>0)
			{
	        	...
	        	for(let i = 0;i<score.length;i++)
	        	{
	        		...
	        		for(let j=0;j<9;j++)
        			{
	        			dojo.place(this.format_block('jstpl_score', {top: 40+ 85 * j, left: 172 + 84 * i, text:score[i][j]}), 'scoreinside2');
        			}
	        	}
			}
        	
        	this.addTooltipHtmlToClass('remove2app',_('When someone places a worker here, the 2 left-most Apprentices (and any coins on them) get removed from the game. Then, all of the Apprentices slide along and 2 more Apprentices get drawn from the deck.'),1000);
			this.addTooltipHtmlToClass('virtue', _("<b>Virtue Track</b><br/>Depending on how much Virtue they have at the game’s end, they may gain or lose Victory Points. This is equal to the numbers shown in the yellow flags next to the upper and lower levels of the Virtue Track."),1000);
        ...
        onEnteringState: function( stateName, args )
        {
            ...
            case 'playerTurn':
	            ...
	            else
	    		{
	            	...
	            	{
	    			$('pagemaintitletext').innerHTML = this.format_string_recursive(_(args.args.title).replace('${actplayer}', this.divActPlayer()).replace('#nb#',args.args.nb).replace('#nb2#',args.args.nb2), args.args);  
	            	}
	            }
            	break;
            }
 		                		...
 		                		else if(args.storehouse != null)
 		                		{
	 		                		...
	 		                		for(var i=1;i<=10;i++)
	 		                		{
	 		                			...
	 		                			if(isolated>0)
	 		                			{
	 		                				...
	 		                				lbl += this.format_block('jstpl_reslog', { id: 0, type:i });
	 		                			}
	 		                		}
        ...
        addDebt:function(debt)
        {
        	...
        	debt.paid = 1;
        	html += this.format_block('jstpl_debt', debt)+'</div><div>'+_("<b>Debt :</b>Unpaid debt make you loose 2 VP or pay it to gain 1 Virtue")+'</div></div>';
        ...
        addBuilding:function(card)
        {
            ...
            dojo.place(this.format_block('jstpl_building', card), $(card.card_location));  
	       	 ...
	       	 var html = '<div class="anatooltip"><div class="anattbuilding">'+this.format_block('jstpl_building', card)+'</div><div class="ttbuilding"><b>'+_(this.helper.buildings[card['card_type']]['name'])+' :</b><br/><div>'+_(this.helper.buildings[card['card_type']]['tooltip'])+'</div></div></div>';
        ...
        addApprentice:function(card)
        {
	        ...
	        dojo.place(this.format_block('jstpl_apprentice', card), $(card.card_location));  
	       	 ...
	       	 var html = '<div class="anatooltip"><div class="anattbuilding">'+this.format_block('jstpl_apprentice', card)+'</div><div class="ttbuilding"><b>'+_(this.helper.apprentices[card['card_type']]['name'])+' :</b><br/><div>'+_(this.helper.apprentices[card['card_type']]['tooltip'])+'</div></div></div>';
        ...
        {        	 
             ...
             {   return; }
              ...
              if(this.selected != null)
            	  {
      			dojo.query(".selectable").removeClass("selectable");
            	  this.ajaxcall( "/architectsofthewestkingdom/architectsofthewestkingdom/actSelect.html", { 
                      lock: true,
                      arg1: this.selected,
        ...
        */
        ...
        {
			dojo.query("#score").removeClass("hidden");
        	dojo.place(this.format_block('jstpl_score', {top: 40+ 85 * notif.args.j, left: 172 + 84 * notif.args.i, text:notif.args.score}), $('scoreinside2'));
        },
        ...
        notif_blackmarket: function(notif)
        {
    		...
    		{
    			dojo.place(this.format_block('jstpl_bmcard', {id:notif.args.blackmarket1.id, posx:(notif.args.blackmarket1.type-1) *11.11, posy:50  }), $("phblackmarket1")); 
    	    }
    		if(notif.args.blackmarket2 != null)
    		{
    			 dojo.place(this.format_block('jstpl_bmcard', {id:notif.args.blackmarket2.id, posx:(notif.args.blackmarket2.type-1) *11.11, posy:0  }), $("phblackmarket2")); 
    	    }
        ...
        notif_reward: function(notif)
        {
            ...
            if(reward != null)
            {
            	dojo.place(this.format_block('jstpl_reward', {id:reward.id, posx:(reward.type-1)%6 *20, posy:100 * Math.floor((reward.type-1)/6)  }), $("phreward")); 
            	this.fadeOutAndDestroy("reward"+reward.id,1000,5000);
            }
            if(notif.args.rewardnb>0)
            {
            	dojo.query('#rewardcpt').forEach(function(selectTag){ selectTag.innerHTML = notif.args.rewardnb; }); 
            }
            else
            {
				dojo.query("#rewardcpt").forEach(dojo.destroy); 
        ...
        sortChildrenDivsById: function(parentId) {
            ...
            ids.sort(function(str1, str2) {return ( ( str1 == str2 ) ? 0 : ( ( str1 > str2 ) ? 1 : -1 ) );});
        ...
        notif_move: function( notif )
        {
    		...
    		if(dojo.hasClass(notif.args.parent,"meeplelocation")
        ...
        notif_counter: function(notif)
        {
       	 	...
       	 	{
       	 		dojo.query("."+notif.args.id).forEach(function(selectTag){ selectTag.innerHTML += "/6" ; }); 
       	 	}
        ...
        {
       	 	dojo.query("#silverbonus"+notif.args.id).forEach(function(selectTag){ selectTag.innerHTML = notif.args.nb; });
       	 	if(notif.args.nb>0)
       	 	{
