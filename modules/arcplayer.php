...
class ARCPlayer extends APP_GameClass
{    
    ...
    function argactionRound($parg1, $parg2)
    {
        ...
        if(self::getUniqueValueFromDB( "select count(*) from worker where location = 'prison_{$this->player_id}'") > 0)
        {
            $ret['selectable']["actguardhouse1"] = array();
        }
        
        if(self::getUniqueValueFromDB( "select count(*) from worker where location = 'prison' and player_id={$this->player_id}") > 0)
        {
        ...
        if($this->checkCost($this->getCost("guardhouse", 4)) && self::getUniqueValueFromDB( "select count(*) from debt where paid = 0 and player_id={$this->player_id}") > 0)
        {
            $ret['selectable']["actguardhouse4"] = array();
        }
        
        if(self::getUniqueValueFromDB( "select count(*) from worker where location='mines' and player_id={$this->player_id}") > 0)
        {
        ...
        if($this->virtue<10)
        {        
            if(self::getUniqueValueFromDB( "select count(*) from worker where location='blackmarketa'") == 0 && $this->checkCost($this->getCost("blackmarketa")))
            {
                $ret['selectable']["actblackmarketa"] = array();
            }
            if(self::getUniqueValueFromDB( "select count(*) from worker where location='blackmarketb'") == 0 && $this->checkCost($this->getCost("blackmarketb")))
            {
                $ret['selectable']["actblackmarketb1"] = array();
                $ret['selectable']["actblackmarketb2"] = array();
            }
            if(self::getUniqueValueFromDB( "select count(*) from worker where location='blackmarketc'") == 0 && $this->checkCost($this->getCost("blackmarketc")))
            {
    ...
    function actionRound($parg1, $parg2, $varg1, $varg2) {
        ...
        $worker_id = self::getUniqueValueFromDB( "select min(id) from worker where player_id = {$this->player_id} and location like 'reserve%'");
        self::DbQuery("update worker set location = '{$action}', location_arg=0 where id = {$worker_id}");
        ...
        $nbmeeplesOnLocation = self::getUniqueValueFromDB( "select count(*)  from worker where player_id = {$this->player_id} and location = '{$action}'");
        ...
        switch($action)
        {
            ...
            case "workshop":
                ...
                else
                {
                    ...
                    ArchitectsOfTheWestKingdom::$instance->notifyAllPlayers( "counter", clienttranslate('${player_name} gains ${nbdiff} <div class="arcicon res9"></div>'), array(
                        ...
                        'nb' => self::getUniqueValueFromDB( "select count(*) from building where card_location = 'hand{$this->player_id}'"),
                        'nbdiff' => $nbcards
                    ) );
                ...
                break;
            case "storehouse":
                if(self::getUniqueValueFromDB( "select count(*)  from apprentice where card_location = 'cards{$this->player_id}' and card_type = 42")>0)
                {
                    $nbmeeplesOnLocation++;                    
                }
            ...
            case "guildhall":
                ...
                break;
            case "blackmarketa":
                $nbplayers = self::getUniqueValueFromDB( "select count(*) from worker where location like 'blackmarket%'");
                if($nbplayers == 3)
                {
    ...
    {
        $action = "guildhall";
        $nbplayers = max(2,self::getUniqueValueFromDB( "select count(*) from player"));
        $nbmeeplesOnLocation = self::getUniqueValueFromDB( "select count(*)  from worker where location = 'guildhall'");
       ...
       self::DbQuery("update worker set location = '{$action}', location_arg={$nbmeeplesOnLocation} where id = {$worker_id}");
        
        if($nbmeeplesOnLocation == $nbresApp1 || $nbmeeplesOnLocation == $nbresApp2 || $nbmeeplesOnLocation == $nbresApp3)
        {
            $apprentice = self::getObjectFromDB("SELECT * FROM apprentice WHERE card_location = 'phapprentice1'");
            ...
            $apprentice = self::getObjectFromDB("SELECT * FROM apprentice WHERE card_location = 'phapprentice2'");
            ArchitectsOfTheWestKingdom::$instance->apprentices->insertCardOnExtremePosition( $apprentice['card_id'], "deck", false );
            ArchitectsOfTheWestKingdom::$instance->notifyAllPlayers( "remove", '', array(
                'id' => "apprentice".$apprentice['card_id']
            ) );
            ...
            for($i=3;$i<=8;$i++)
            {
                $apprentice = self::getObjectFromDB("SELECT * FROM apprentice WHERE card_location = 'phapprentice{$i}'");
                $newapprenticeIndex = intval($i) - 2;
                
                self::DbQuery("update apprentice set card_location = 'phapprentice{$newapprenticeIndex}' where card_id = {$apprentice['card_id']}");
            ...
            ArchitectsOfTheWestKingdom::$instance->setGameStateValue( 'no_undo', 1);
            $apprentice = self::getObjectFromDB("SELECT * FROM apprentice WHERE card_location = 'phapprentice{$newloc}' ");
    ...
    function guardhouse3($parg1, $parg2, $varg1, $varg2)
    {
        ...
        {  
            ...
            self::DbQuery("update worker set location = '{$target}', location_arg=0 where location like 'prison_%' and player_id = {$worker['player_id']}");
            ...
            foreach($players as $player)
            {
                ArchitectsOfTheWestKingdom::$instance->notifyAllPlayers( "counter", '', array(
                    ...
                    'nb' => self::getUniqueValueFromDB("SELECT count(*) FROM worker WHERE location = 'prison_{$player['player_id']}'")
                ) );
            }
    ...
    function guardhouse1()
    {
        ...
        self::DbQuery("update worker set location = '{$target}', location_arg=0 where location = 'prison_{$this->player_id}'");          
    ...
    function guardhouse2()
    {
        ...
        self::DbQuery("update worker set location = '{$target}', location_arg=0 where location = 'prison' and player_id = {$this->player_id}");
    ...
    {        
        ...
        self::DbQuery("update worker set location = '{$target}', location_arg=0 where location like 'blackmarket%'");  
    ...
    function debtRefund()
    {
        $debt = self::getObjectFromDB("SELECT * FROM debt WHERE player_id = {$this->player_id} and paid = 0 limit 1");
        if($debt != null)
        {
            self::DbQuery("update debt set paid = 1 where id = {$debt['id']}");
    ...
    function argguildhall($parg1, $parg2)
    {
        ...
        {        
            $spotFilled = self::getUniqueValueFromDB( "select count(*) from player where cathedral = 1+ {$this->cathedral}");
    ...
    function guildhall($parg1, $parg2, $varg1, $varg2)
    {
        ...
        {          
            ...
            else
            {
                ...
                $building = self::getObjectFromDB("SELECT * FROM building WHERE card_id = {$buildingId}");
                $objbuilding = building::instantiate($building);
    ...
    function cathedral($parg1, $parg2, $varg1, $varg2)
    {
        ...
        self::DbQuery("UPDATE player SET cathedral = {$this->cathedral} where player_id = {$this->player_id}");
    ...
    {   
       ...
       if($nbmeeplesOnLocation < $nbmax)
        ...
        { 
            ...
            self::DbQuery("update worker set location = '{$target}', location_arg=0 where id = {$workerId}");
    ...
    function towncenter($parg1, $parg2, $varg1, $varg2)
    {
        ...
        {       
            ...
            $workers = ArchitectsOfTheWestKingdom::$instance->getCollectionFromDb( $sql ); 
            $other_player_name = self::getUniqueValueFromDB("select player_name from player where player_id = {$worker['player_id']}");
            ...
            self::DbQuery("update worker set location = '{$target}', location_arg=0 where location = '{$worker['location']}' and player_id = {$worker['player_id']}");   
            ...
            else
            {
                ArchitectsOfTheWestKingdom::$instance->notifyAllPlayers( "counter", '', array(
                    'id' => "res_".$this->player_id."_12",
                    'nb' => self::getUniqueValueFromDB("SELECT count(*) FROM worker WHERE location = 'prison_{$this->player_id}'")
                ) );
            }
    ...
    function workshop($parg1, $parg2, $varg1, $varg2)
    {
        ...
        { 
            ...
            $apprenticeIndex = (int) filter_var(self::getUniqueValueFromDB("SELECT card_location FROM apprentice WHERE card_id = {$apprenticeId}"), FILTER_SANITIZE_NUMBER_INT);
            ...
            if($extracoin>0)
            {
                ...
                for($i=0;$i<$extracoin;$i++)
                {
                    ...
                    $apprentice = self::getObjectFromDB("SELECT * FROM apprentice WHERE card_location = 'phapprentice{$index}'");
                    $bonus = $apprentice['bonus'] + 1;
                    
                    self::DbQuery("update apprentice set bonus = {$bonus} where card_id = {$apprentice['card_id']}");                    
    ...
    function discardApprentice($parg1, $parg2, $varg1, $varg2)
    {
        if($varg1 != "Skip" && $varg1 != null)
        {
            ...
            $apprentice = self::getObjectFromDB("SELECT * FROM apprentice WHERE card_id = {$apprenticeId}");
    ...
    function pickApprentice($parg1, $parg2, $varg1, $varg2)
    {
        if($varg1 != 'Skip' && $varg1 != null)
        {
            ...
            {            
                ...
                self::DbQuery("update apprentice set bonus = 0 where card_id = {$apprentice['card_id']}");
                ArchitectsOfTheWestKingdom::$instance->notifyAllPlayers( "countermask", '', array(
                    'id' => $apprentice['card_id'],
                    'nb' => 0
                ) );
            ...
            else if($type == VIRTUE)
            {
                ...
                if($this->virtue>14)
                {
                    for($i=0;$i<$this->virtue-14;$i++)
                    {
                        ...
                        if($debt != null)
                        {
                            self::DbQuery("delete from debt where id = {$debt['id']}");                          
                ...
                self::DbQuery("update player set virtue = ".$this->virtue." where player_id = ".$this->player_id);
            ...
            for($type=1;$type<=10;$type++)
            {
                ...
                if($nb>0)
                {
                    ...
                    else if($type==7) //tax
                    {
                        ...
                        self::DbQuery("update player set res6 = ".$this->resources[SILVER]." where player_id = ".$this->player_id);
                    ...
                    else if($type==DEBT) //Debt
                    {
                        for($i=0;$i<$nb;$i++)
                        {
                            ...
                            {
                                self::DbQuery("insert into debt (player_id) VALUES ({$this->player_id})");
    ...
    function updateVP($final = false)
    {
        ...
        $malus = -2;
        if(self::getUniqueValueFromDB( "SELECT count(*) from building where card_location = 'cards{$this->player_id}' and card_type = 27")>0)
        {
            $malus = -1;
        }
        
        $vp += $malus * self::getUniqueValueFromDB( "select count(*) from debt where player_id = {$this->player_id} and paid = 0");
        $ret[4] = $malus * self::getUniqueValueFromDB( "select count(*) from debt where player_id = {$this->player_id} and paid = 0");
        ArchitectsOfTheWestKingdom::$instance->setStat($malus * self::getUniqueValueFromDB( "select count(*) from debt where player_id = {$this->player_id} and paid = 0"), 'debt', $this->player_id);
        ...
        $vp -= intdiv(intval(self::getUniqueValueFromDB( "select count(*) from worker where player_id = {$this->player_id} and location ='prison'")),2);
        $ret[7] = -1* intdiv(intval(self::getUniqueValueFromDB( "select count(*) from worker where player_id = {$this->player_id} and location ='prison'")),2);
        ArchitectsOfTheWestKingdom::$instance->setStat(-1*intdiv(intval(self::getUniqueValueFromDB( "select count(*) from worker where player_id = {$this->player_id} and location ='prison'")),2), 'prison', $this->player_id); 
        ...
        {
            self::DbQuery("UPDATE player SET player_score = {$vp}, player_score_aux = {$aux} where player_id = {$this->player_id}");
    ...
    function argfara($parg1, $parg2)
    {
        ...
        $prisoners = self::getUniqueValueFromDB( "SELECT count(*) from worker where location = 'prison' and player_id = {$this->player_id}");
        if($prisoners>0)
        {
            $ret['buttons'][] = 'Confirm';
            $ret['selectable']['Confirm'] = array();
