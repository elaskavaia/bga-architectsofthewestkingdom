...
class building41 extends building
{
    ...
    public function instant(ARCPlayer $player)
    {
        ...
        //ArchitectsOfTheWestKingdom::$instance->debugLog(Json::encode($arg));
        if (!$arg['void']) {
            ...
            ArchitectsOfTheWestKingdom::$instance->notify->all("message", clienttranslate('${player_name} skips bonus for ${location}: cannot afford'), [
                'player_id' => $player->player_id,
                'location' => "guildhall"
            ]);
        }
